...
const department_id = JSON.parse(
  document.getElementById("department_id").textContent
);
const location_id = JSON.parse(
  document.getElementById("location_id").textContent
);

const dept_name = JSON.parse(document.getElementById("dept_name").textContent);
const loc_name = JSON.parse(document.getElementById("loc_name").textContent);
...
$(document).ready(function () {
  ...
  $("#month-filter, #year-filter, #user-filter, #project-filter").change(
    function () {
      handleFilterChange();
    }
  );
  ...
  function downloadExcel(activityLogs, name) {
    ...
    link.download = `activity_logs_${name}_${usr}_${date}_loc(${loc})_dept(${dept}).csv`; // Change the file extension to .csv
